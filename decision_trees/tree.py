...
def make_tree(data,target,features,idx='0',last_best_feature_value='-1'):
    ...
    for idxs,value in enumerate(data[best_feature].unique()):
        ...
        tree[best_feature][str(value)] = make_tree(subset, target, features,idx+str(idxs),str(value)+'_' + idx+str(idxs))
    #else make divisions
